// --- Boards (Kanban) page ---
//
// Column-based issue/task view over configured boards (GitLab/GitHub/local).
// Subheader owns board picker + search + filters; body holds the kanban
// columns + issue modal + agent activity drawer. Client JS drives sortable
// drag-drop, live updates, and optimistic writes.
//
// Deduplicated from the old board-dashboard.ts monolith:
//   ui/tokens.ts         — :root, theme overrides, font/color vars
//   ui/components.css.ts — .ax-badge / .ax-card / .ax-field reusable bits
//   topbar.ts            — .ax-topbar / .ax-subheader / .ax-theme-switch
//   HERE                 — subheader inputs + kanban columns + issue modal +
//                          activity drawer + client JS
//
// The page accepts renderBoardPage({ peers }) — daemon-config awareness
// stays in board-dashboard.ts where the routing happens.

import { renderShell, type TopbarPeer } from ".."

export interface BoardsPageOpts {
  peers?: TopbarPeer[]
  currentPeerId?: string
}

export function renderBoardsPage(opts: BoardsPageOpts = {}): string {
  const subheader = `
  ...
  const body = `<aside id="activity-panel" class="activity-panel hidden" aria-hidden="true">
  <header>
...
<script src="/sortable.min.js"></script>`
  ...
  return renderShell({
    ...
    body,
    css: BOARDS_PAGE_CSS,
    scripts: `<script>${BOARDS_PAGE_SCRIPT}</script>`,
    headExtras: `<script>
      // The boards page has its own connection dot + wants body height:100%
      // for the full-viewport kanban layout. Inject the dot into the topbar
      // and override body height at runtime so the shared shell doesn't
      // need a dedicated opt-in.
      document.addEventListener('DOMContentLoaded', () => {
        document.documentElement.style.height = '100%';
        document.body.style.height = '100%';
        const right = document.querySelector('.ax-topbar__right');
        if (right && !right.querySelector('.conn')) {
          const dot = document.createElement('div');
          dot.id = 'conn'; dot.className = 'conn ok'; dot.title = 'ready';
          dot.textContent = '●';
