...
//   POST /talk/stop
//   POST /teach/live  {agent, goal, mode?: teach | watch | act, app?}  a live lesson
...
export class VoiceTalkService {
  ...
  handle(method: string, path: string, body: Record<string, unknown>): Reply {
    ...
    switch (`${method} ${path}`) {
      ...
      case "POST /teach/live": {
        ...
        // No app named: the one in front when the lesson starts.
        const app = typeof body.app === "string" && body.app.trim() ? body.app.trim() : null
        return this.startLesson(agentId, goal, mode, app)
      }
      case "POST /talk/hush":
  ...
  startLesson(agentId: string, goal: string, mode: TeachMode, app?: string | null): Reply {
