...
Returns your full memory list + the MEMORY.md block that gets inlined in your prompt.
...
Before saving, quickly scan the existing memory (call the GET endpoint or look at the inlined MEMORY.md in your own system prompt). If a similar entry exists, prefer updating it over creating a new one — the index gets noisy otherwise.
