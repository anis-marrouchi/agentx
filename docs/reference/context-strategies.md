...
They apply to native-session CLI tiers: `claude-code` (`claudeSessionId`), `codex-cli` (`codexSessionId`), and `opencode` (`opencodeSessionId`). `sdk` and `orchestrator` use AgentX-rendered bounded history instead of a native CLI resume id, so the stale/max-turn/tier-2 native-session rotation knobs do not apply to them.
